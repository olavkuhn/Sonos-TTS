...
class ConfigNotComplete(Exception):
    """Exception raised when (currently) there is no IP address set in config.toml"""

    def __init__(self):
        super().__init__(f"There is no IP address set in config.toml")
